...
public class WeeklyStatisticsController {
    ...
    @GetMapping("/weekly/summury")
    public ApiResponse<WeeklyStatisticsResponse> getWeeklySummary(@RequestParam Long managerId) {
        // 보통 SecurityContext에서 managerId를 꺼내거나,
        // 편의상 파라미터로 받는 예시
        WeeklyStatisticsResponse data = weeklyStatisticsService.getWeeklySummary(managerId);
